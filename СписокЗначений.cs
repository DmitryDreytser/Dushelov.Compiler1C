// Type: Compiler1C.СписокЗначений
...
{
  internal static class СписокЗначений
  {
    internal static void Code(CodeStruct node, ref string code, Объект1С lst, ref int index)
    {
      ...
      {
        if (!(объект1С1.Имя.ToLower() == ((object) СписокЗначений.Методы.получить).ToString()))
          return;
        // ISSUE: explicit reference operation
        // ISSUE: variable of a reference type
        string& local2 = @code;
          ...
          else
          {
            ...
            if (num == 0)
            {
              ...
              if (index1 + 2 > node.Objects.Count || !(((Объект1С) node.Objects[index1 + 1]).Имя == "."))
                break;
              СписокЗначений.ЭлементСпискаЗначений.Code(node, ref code, (Объект1С) null, ref index);
              break;
            }
          }
        }
    ...
    internal static class ЭлементСпискаЗначений
    {
      internal static void Code(CodeStruct node, ref string code, Объект1С elem, ref int index)
      {
        ...
        if (index + 1 > node.Objects.Count || !(((Объект1С) node.Objects[index + 1]).Имя == ".") || !(((Объект1С) node.Objects[index + 2]).Имя.ToLower() == ((object) СписокЗначений.ЭлементСпискаЗначений.Свойства.значение).ToString()))
          return;
        index = index + 2;
      }
